...
def upgrade() -> None:
    ...
    op.add_column(
        "processing_jobs",
        sa.Column("current_stage", sa.String(length=20), nullable=False, server_default="uploaded"),
    )
    op.create_index(
        op.f("ix_processing_jobs_current_stage"), "processing_jobs", ["current_stage"], unique=False
    )
    ...
    op.add_column(
        "processing_jobs", sa.Column("last_heartbeat", sa.DateTime(timezone=True), nullable=True)
    )
